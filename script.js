...
// fetch.phpの方に書くとundefinedになってしまう。・・・なんで？
//
function search() {
    // 検索フォームから名前を取得
...
function displayResults(results) {
    ...
    for (var i = 0; i < results.length; i++) {
        table += "<tr onclick='selectRow(" + results[i].id + ")'><td>" + results[i].id + "</td><td>" + results[i].name + "</td><td>" + results[i].date + "</td></tr>";
    }
...
function selectRow(id) {
    ...
    // ここではalertで代用しますが、実際のアプリケーションでは他の方法で表示することができます
    modal.style.display = "none";
